...
export interface ClientOptions {
   ...
   * - `production` corresponds to `https://api.case.dev`
   * - `local` corresponds to `http://localhost:2728`
   */
   ...
   *
   * Defaults to process.env['CASEDEV_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CASEDEV_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Casedev API.
 */
export class Casedev {
  apiKey: string;
   ...
   * API Client for interfacing with the Casedev API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['CASEDEV_API_KEY'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['CASEDEV_BASE_URL'] ?? https://api.case.dev] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CASEDEV_BASE_URL'),
    apiKey = readEnv('CASEDEV_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CasedevError(
        "The CASEDEV_API_KEY environment variable is missing or empty; either provide it, or instantiate the Casedev client with an apiKey option, like new Casedev({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Casedev.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CASEDEV_LOG'), "process.env['CASEDEV_LOG']", this) ??
      defaultLogLevel;
  ...
  static CasedevError = Errors.CasedevError;
